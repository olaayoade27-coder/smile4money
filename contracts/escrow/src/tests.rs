...
fn test_create_match_duplicate_game_id_fails() {
    let (env, contract_id, _oracle, player1, player2, token, _admin) = setup();
    let client = EscrowContractClient::new(&env, &contract_id);
    client.create_match(
        &player1, &player2, &100, &token,
...
#[test]
fn test_is_funded_partial_deposit() {
    let (env, contract_id, _oracle, player1, player2, token, _admin) = setup();
    let client = EscrowContractClient::new(&env, &contract_id);

    let id = client.create_match(
        &player1, &player2, &100, &token,
        &String::from_str(&env, "partial"), &Platform::Lichess,
    );
...
fn test_escrow_balance_stages() {
    ...
    let id = client.create_match(
        &player1, &player2, &100, &token,
        &String::from_str(&env, "stages"), &Platform::Lichess,
    );
...
#[test]
fn test_non_player_cannot_deposit() {
    let (env, contract_id, _oracle, player1, player2, token, _admin) = setup();
    let client = EscrowContractClient::new(&env, &contract_id);
    ...
    assert_eq!(client.try_deposit(&id, &stranger), Err(Ok(Error::Unauthorized)));
}
...
#[test]
fn test_submit_result_game_id_mismatch_fails() {
    let (env, contract_id, oracle, player1, player2, token, _admin) = setup();
    let client = EscrowContractClient::new(&env, &contract_id);
...
fn test_update_oracle() {
    ...
    // New oracle can now submit results
    let id = client.create_match(
        &player1, &player2, &100, &token,
        &String::from_str(&env, "oracle_update"), &Platform::Lichess,
    );
    client.deposit(&id, &player1);
    client.deposit(&id, &player2);
    client.submit_result(&id, &String::from_str(&env, "oracle_update"), &Winner::Player1, &new_oracle);
    assert_eq!(client.get_match(&id).state, MatchState::Completed);
}
...
fn test_old_oracle_rejected_after_update() {
    ...
    let id = client.create_match(
        &player1, &player2, &100, &token,
        &String::from_str(&env, "old_oracle"), &Platform::Lichess,
    );
    client.deposit(&id, &player1);
    client.deposit(&id, &player2);

    assert_eq!(
        client.try_submit_result(&id, &String::from_str(&env, "old_oracle"), &Winner::Player1, &oracle),
        Err(Ok(Error::Unauthorized))
    );
}
